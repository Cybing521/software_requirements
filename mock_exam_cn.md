...
**场景:**
项目正如火如荼地进行，但客户不断更改需求。团队迷失了方向，不知道哪段代码对应哪个需求。

**要求:**
1.  定义**需求可追溯性矩阵 (RTM)** 并解释它的两个主要好处（例如：用于变更影响分析）。
2.  **变更控制委员会 (CCB)** 的职责是什么？
3.  使用 Mermaid 绘制**变更控制流程**（如：提出请求 -> 评估 -> 决策 -> 实施 -> 验证）。

### 答案

**1. 可追溯性矩阵:**
*   一种将用户需求链接到功能需求，然后再链接到设计、代码和测试用例的文档（通常是表格）。
*   **好处:**
    1.  **变更影响分析:** 当需求变更时，快速识别哪些模块需要更新。
    2.  **覆盖率分析:** 确保所有需求都已实现并经过测试（没有遗漏的功能）。

**2. CCB 角色:**
*   由利益相关者（项目经理、BA、客户、技术主管）组成的各种小组，根据范围、成本和可行性决定是否**批准**或**拒绝**提议的变更。
...
graph LR
    Req[变更请求] --> Eval[评估影响]
    Eval --> Decision{CCB 决策}
    Decision -- 批准 --> Impl[实施变更]
    Decision -- 拒绝 --> End((结束))
    Impl --> Verify[验证测试]
    Verify --> Update[更新文档]
```
...
**场景:**
上一个项目因为“范围蔓延” (Scope Creep) 而失败。团队希望为下一个项目改进流程。

**要求:**
1.  使用**根本原因分析 (Five Whys)** 解释如何找到问题的真正原因（举一个简短的例子）。
2.  定义**范围蔓延 (Scope Creep)** 以及一种预防方法。
3.  绘制**风险管理流程**图（识别 -> 分析 -> 排序 -> 处理 -> 监控）。

### 答案

**1. 根本原因分析 (Five Whys):**
*   一种通过至少问5次“为什么”来深入挖掘根本原因的技术。
*   *例子：项目延期？-> 为什么？（变更太多）-> 为什么？（需求不明确）-> 为什么？（用户未参与）-> 根本原因：缺乏用户参与。*

**2. 范围蔓延 (Scope Creep):**
*   **定义:** 在没有调整时间、成本或资源的情况下，随着需求的增加，项目范围不受控制地增长。
*   **预防:** 严格通过**变更控制流程**（所有变更必须经过CCB）。

**3. 风险管理流程:**

```mermaid
graph TD
    ID[识别风险] --> Analyze[分析 (可能性 x 影响)]
    Analyze --> Rank[排序 & 优先级]
    Rank --> Handle[处理 (减轻/避免)]
    Handle --> Monitor[监控 & 审查]
    Monitor -.-> ID
```
